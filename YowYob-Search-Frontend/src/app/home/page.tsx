  ...
  const handleSearch = () => {
    if (searchQuery.trim()) {
      if (status === 'authenticated') {
        // Persist independently of navigation to ensure capture
        // We import searchService dynamically or use the one from lib
        import('@/lib/api/search-service').then(({ searchService }) => {
